...
u64 *process_envelope(u64 *cmd, struct Note *note, u32 nSamples, u16 inBuf, s32 headsetPanSettings);
...
u64 *synthesis_execute(u64 *cmdBuf, s32 *writtenCmds, s16 *aiBuf, s32 bufLen) {
    ...
    for (i = gAudioUpdatesPerFrame; i > 0; i--) {
        ...
        cmd = synthesis_do_one_audio_update((s16 *) aiBufPtr, chunkLen << 1, cmd, gAudioUpdatesPerFrame - i);
...
u64 *process_envelope(u64 *cmd, struct Note *note, u32 nSamples, u16 inBuf, s32 headsetPanSettings) {
    s32 rampLeft;
    s32 rampRight;
    u16 sourceLeft = note->curVolLeft;
    u16 sourceRight = note->curVolRight;
    u16 targetLeft = note->targetVolLeft;
    u16 targetRight = note->targetVolRight;
    u8 mixerFlags;
    ...
    if (targetLeft == sourceLeft && targetRight == sourceRight && !note->envMixerNeedsInit) {
        ...
        rampLeft = currentRampingTableLeft[targetLeft >> 8] * currentRampingTableRight[sourceLeft >> 8];
        rampRight = currentRampingTableLeft[targetRight >> 8] * currentRampingTableRight[sourceRight >> 8];
