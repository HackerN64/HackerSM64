...
void bhv_goomba_triplet_spawner_update(void) {
    ...
    if (o->oAction == GOOMBA_TRIPLET_SPAWNER_ACT_UNLOADED) {
        if (o->oDistanceToMario < 3000.0f) {
            ...
            o->oAction = GOOMBA_TRIPLET_SPAWNER_ACT_LOADED;
        }
...
static void goomba_act_walk(void) {
    ...
    //! By strategically hitting a wall, steep slope, or another goomba, we can prevent
    //  the goomba from turning back toward home for a while (goomba chase extension)
    //! It seems theoretically possible to get 2-3 goombas to repeatedly touch each
    //  other and move arbitrarily far from their home, but it's extremely precise and
    //  chaotic in practice, so probably can't be performed for nontrivial distances
    if (o->oGoombaTurningAwayFromWall) {
        ...
        o->oGoombaTurningAwayFromWall = obj_bounce_off_walls_edges_objects(&o->oGoombaTargetYaw);
        if (!o->oGoombaTurningAwayFromWall) {
            if (o->oDistanceToMario < 500.0f) {
                // If close to mario, begin chasing him. If not already chasing him, jump first
...
static void goomba_act_jump(void) {
    ...
    //! If we move outside the goomba's drawing radius the frame it enters the jump action,
    //  then it will keep its velY, but it will still be counted as being on the ground.
    //  Next frame, the jump action will think it has already ended because it is still on the ground.
