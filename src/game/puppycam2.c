...
void puppycam_default_config(void) {
    ...
    gPuppyCam.options.inputType      = PUPPYCAMP_INPUT_TYPE_SINGLE_PRESS;
}
...
s32 puppycam_move_spline(struct sPuppySpline splinePos[], struct sPuppySpline splineFocus[], s32 mode, s32 index) {
    ...
    for (i = 0; i < 4; i++) {
        vec3f_set(tempPoints[i], splinePos[gPuppyCam.splineIndex + i].pos[0],
                                 splinePos[gPuppyCam.splineIndex + i].pos[1],
                                 splinePos[gPuppyCam.splineIndex + i].pos[2]);
    }
    puppycam_evaluate_spline(gPuppyCam.splineProgress, gPuppyCam.pos, tempPoints[0],
                                                                      tempPoints[1],
                                                                      tempPoints[2],
                                                                      tempPoints[3]);
    if (mode == PUPPYSPLINE_FOLLOW) {
        for (i = 0; i < 4; i++) {
            vec3f_set(tempPoints[i], splineFocus[gPuppyCam.splineIndex + i].pos[0],
                                     splineFocus[gPuppyCam.splineIndex + i].pos[1],
                                     splineFocus[gPuppyCam.splineIndex + i].pos[2]);
        }
        puppycam_evaluate_spline(gPuppyCam.splineProgress, gPuppyCam.focus, tempPoints[0],
                                                                            tempPoints[1],
                                                                            tempPoints[2],
                                                                            tempPoints[3]);
    }
...
void puppycam_change_setting(s8 toggle) {
    ...
    if ((gPCOptions[gPCOptionSelected].gPCOptionMin == FALSE)
     && (gPCOptions[gPCOptionSelected].gPCOptionMax == TRUE)) {
...
void puppycam_input_pitch(void) {
    if (gPuppyCam.flags & PUPPYCAM_BEHAVIOUR_PITCH_ROTATION) {
        ...
        if ((gPlayer1Controller->buttonDown & U_CBUTTONS) || (gPuppyCam.stick2[1] != 0)) {
            gPuppyCam.pitchAcceleration -= (50 * (gPuppyCam.options.sensitivityY / 100.0f));
...
static void puppycam_input_hold_preset1(f32 ivX) {
    ...
    if (((gPlayer1Controller->buttonDown & L_CBUTTONS) && !gPuppyCam.options.analogue) || (gPuppyCam.stick2[0] != 0)) {
        gPuppyCam.yawAcceleration -= (75 * (gPuppyCam.options.sensitivityX / 100.0f));
...
static void puppycam_input_hold_preset3(void) {
    ...
    if (gPuppyCam.mode3Flags & PUPPYCAM_MODE3_ZOOMED_IN) {
        ...
        if (abss(gPlayer1Controller->rawStickX) > DEADZONE) {
            gPuppyCam.yawAcceleration   -= ((gPuppyCam.options.sensitivityX / 100.0f) * stickMag[0]);
        } else {
            gPuppyCam.yawAcceleration   = 0;
        }
        if (abss(gPlayer1Controller->rawStickY) > DEADZONE) {
            gPuppyCam.pitchAcceleration -= ((gPuppyCam.options.sensitivityY / 100.0f) * stickMag[1]);
        } else {
        ...
        if (gPuppyCam.mode3Flags & PUPPYCAM_MODE3_ZOOMED_MED) gPuppyCam.pitchTarget = approach_s32_symmetric(gPuppyCam.pitchTarget, 0x3800, 0x200);
        if (gPuppyCam.mode3Flags & PUPPYCAM_MODE3_ZOOMED_OUT) gPuppyCam.pitchTarget = approach_s32_symmetric(gPuppyCam.pitchTarget, 0x3000, 0x200);

        if (((gPlayer1Controller->buttonPressed & L_CBUTTONS) && !gPuppyCam.options.analogue)
         || ((gPuppyCam.stick2[0] > DEADZONE) && !gPuppyCam.stickN[0])) {
    ...
    if ((gPlayer1Controller->buttonPressed & U_CBUTTONS && !gPuppyCam.options.analogue)
    ...
    } else if (((gPlayer1Controller->buttonPressed & D_CBUTTONS) && !gPuppyCam.options.analogue)
     || ((gPuppyCam.stick2[1] < -DEADZONE) && !gPuppyCam.stickN[1])) { // Otherwise handle zooming out.
        if (gPuppyCam.mode3Flags & PUPPYCAM_MODE3_ZOOMED_MED) {
            gPuppyCam.stickN[1]   = 1;
            gPuppyCam.mode3Flags |= PUPPYCAM_MODE3_ZOOMED_OUT;
            gPuppyCam.mode3Flags &= ~PUPPYCAM_MODE3_ZOOMED_MED;
...
static void puppycam_input_hold(void) {
    f32 ivX = ((gPuppyCam.options.invertX * 2) - 1) * (gPuppyCam.options.sensitivityX / 100.0f);
    f32 ivY = ((gPuppyCam.options.invertY * 2) - 1) * (gPuppyCam.options.sensitivityY / 100.0f);
    ...
    if ((!gPuppyCam.options.analogue || (gPuppyCam.options.inputType == PUPPYCAM_INPUT_TYPE_CLASSIC)) && !(gPuppyCam.flags & PUPPYCAM_BEHAVIOUR_FREE)) {
        switch (gPuppyCam.options.inputType) {
            ...
            case PUPPYCAMP_INPUT_TYPE_SINGLE_PRESS:  puppycam_input_hold_preset2(ivX); puppycam_input_pitch(); puppycam_input_zoom(); puppycam_input_centre(); break;
...
static void puppycam_view_panning(void) {
    ...
    f32 slideSpeed = 1;
    f32 panMulti = (gPuppyCam.zoom / (f32)gPuppyCam.zoomPoints[2]);
    panMulti = CLAMP(panMulti, 0.0f, 1.0f);
    if (gPuppyCam.options.inputType == PUPPYCAM_INPUT_TYPE_CLASSIC) {
        panMulti /= 2;
    }
    if ((gPuppyCam.flags & PUPPYCAM_BEHAVIOUR_PANSHIFT)
     ...
     && (gMarioState->action != ACT_START_SLEEPING)) {
        ...
        f32 panVel   = (panEx + (200 * (gMarioState->forwardVel / 320.0f)));
        expectedPanX = (LENSIN(panVel, gMarioState->faceAngle[1]) * panMulti);
        expectedPanZ = (LENCOS(panVel, gMarioState->faceAngle[1]) * panMulti);
        ...
        if (gMarioState->vel[1] == 0.0f) {
            ...
            if (panFloor <= gPuppyCam.targetObj->oPosY - 150) {
                ...
                gPuppyCam.edgePitch = approach_s32_symmetric(gPuppyCam.edgePitch, -0x2000,  0x80);
            } else {
                gPuppyCam.edgePitch = approach_s32_symmetric(gPuppyCam.edgePitch,     0x0, 0x100);
            }
...
static s32 puppycam_check_volume_bounds(struct sPuppyVolume *volume, s32 index) {
    ...
    if (sPuppyVolumeStack[index]->shape == PUPPYVOLUME_SHAPE_BOX) {
        ...
        f32 sinrot = sins(sPuppyVolumeStack[index]->rot);
        f32 cosrot = coss(sPuppyVolumeStack[index]->rot);
        ...
        if ((-sPuppyVolumeStack[index]->radius[1] < rel[1])
...
void puppycam_projection_behaviours(void) {
    ...
    if (gPuppyCam.targetObj == gMarioState->marioObj) {
        ...
        if ((gPuppyCam.options.turnAggression > 0)
         ...
         && !(gPuppyCam.flags & PUPPYCAM_BEHAVIOUR_INPUT_4DIR)
         && (gPuppyCam.options.inputType != PUPPYCAM_INPUT_TYPE_CLASSIC)) { // Holy hell this is getting spicy.
            // With turn aggression enabled, or if Mario's sliding, adjust the camera view behind mario.
            if ((gPuppyCam.options.turnAggression > 0) || (action & ACT_FLAG_BUTT_OR_STOMACH_SLIDE)) {
                if (action & ACT_FLAG_BUTT_OR_STOMACH_SLIDE) {
                ...
                if (((abss(gPlayer1Controller->rawStickX) > 20) && !(action & ACT_FLAG_BUTT_OR_STOMACH_SLIDE))
                 || ((action & ACT_FLAG_BUTT_OR_STOMACH_SLIDE) && (s16)abss(((gPuppyCam.yaw + 0x8000) % 0xFFFF - 0x8000) - ((gMarioState->faceAngle[1]) % 0xFFFF - 0x8000)) < 0x3000)) {
                    gPuppyCam.yawTarget = approach_angle(gPuppyCam.yawTarget, (gMarioState->faceAngle[1] + 0x8000), ((gPuppyCam.options.turnAggression * 10) * absf(gMarioState->forwardVel / 32) * absf(gPlayer1Controller->rawStickX / 80.0f) * turnRate));
                }
            }
        ...
        f32 targetFloorHeight = find_floor_height(gPuppyCam.targetObj->oPosX, gPuppyCam.targetObj->oPosY, gPuppyCam.targetObj->oPosZ);
        gPuppyCam.targetFloorHeight = CLAMP(targetFloorHeight, (gPuppyCam.targetObj->oPosY - 350), (gPuppyCam.targetObj->oPosY + 300));
        f32 velY = ((absf(gMarioState->vel[1]) - 17.0f) / 200.0f);
        targetFloorHeight = ((absf(gPuppyCam.targetFloorHeight - gPuppyCam.lastTargetFloorHeight) - 30.0f) / 300.0f);
        gPuppyCam.lastTargetFloorHeight = approach_f32_asymptotic(gPuppyCam.lastTargetFloorHeight, gPuppyCam.targetFloorHeight, (CLAMP(velY, 0, 0.1f) + CLAMP(targetFloorHeight, 0, 0.1f)));

        if ((action == ACT_SLEEPING) || (action == ACT_START_SLEEPING)) {
            gPuppyCam.zoom = approach_f32_asymptotic(gPuppyCam.zoom, gPuppyCam.zoomPoints[0], 0.01f);
        } else if ((action & ACT_FLAG_SWIMMING) && (((gMarioState->waterLevel - 100) - gMarioState->pos[1]) > 5)) {
            // When moving underwater, the camera will zoom in on Mario.
            f32 zoom = (gPuppyCam.zoomTarget / 1.5f);
            gPuppyCam.zoom = approach_f32_asymptotic(gPuppyCam.zoom, MAX(zoom, gPuppyCam.zoomPoints[0]), 0.2f);
        } else {
        ...
        if ((action & ACT_FLAG_MOVING)
         || (action & ACT_FLAG_AIR)
         || ((action & ACT_FLAG_SWIMMING) && !(((gMarioState->waterLevel - 100) - gMarioState->pos[1]) > 5) && (gMarioState->forwardVel != 0.0f))) {
        ...
        if (gMarioState->forwardVel > 0) {
            f32 zoomSpeed = (gMarioState->forwardVel / 10);
            gPuppyCam.moveZoom = approach_f32_symmetric(gPuppyCam.moveZoom, (100.0f * (gMarioState->forwardVel / 32.0f)), zoomSpeed);
        } else {
            gPuppyCam.moveZoom = approach_f32_symmetric(gPuppyCam.moveZoom, 0, 5);
        }

        // Zooms the camera in further when underwater.
        if ((gPuppyCam.pitch > 0x38C0) && (abss(gPuppyCam.swimPitch) < 100)) {
            f32 zoomUnclamped = (f32)((gPuppyCam.pitch - 0x38C0) / 3072.0f);
            gPuppyCam.zoom = approach_f32_asymptotic((f32)gPuppyCam.zoom, 250.0f, CLAMP(zoomUnclamped, 0.0f, 1.0f));
        }

        if (!(gMarioState->action & ACT_FLAG_SWIMMING)) {
            f32 dy = (gPuppyCam.targetObj->oPosY - gPuppyCam.lastTargetFloorHeight);
            gPuppyCam.floorY[0] = softClamp(dy, -180, 300);
            gPuppyCam.floorY[1] = softClamp(dy, -180, 350);
...
static void puppycam_projection(void) {
    ...
    s32 pitchTotalUnclamped = (gPuppyCam.pitch + (gPuppyCam.swimPitch * 10) + gPuppyCam.edgePitch + gPuppyCam.terrainPitch);
    s16 pitchTotal = CLAMP(pitchTotalUnclamped, 800, 0x7800);
    ...
    if (gPuppyCam.targetObj) {
        ...
        if (gPuppyCam.targetObj2) {
            ...
            gPuppyCam.targetDist[0] = approach_f32_asymptotic(gPuppyCam.targetDist[0], (absi(LENCOS(sqrtf(sqr(d[0]) + sqr(d[2])),
                            (s16)abss(((gPuppyCam.yaw + 0x8000) % 0xFFFF - 0x8000) - (atan2s(d[2], d[0])) % 0xFFFF - 0x8000) + 0x4000))), 0.2f);
        } else {
...
static void puppycam_collision(void) {
    ...
    f32 unclampedOpacityDist = (f32)(((gPuppyCam.zoom - END_DIST) / 255.0f) * (START_DIST - END_DIST));
    gPuppyCam.opacity = CLAMP(unclampedOpacityDist, 0, 255);
}
...
static void puppycam_apply(void) {
    ...
    gLakituState.roll    = approach_s32_symmetric(gLakituState.roll, 0x0, 0x80);
