...
s32 tilt_body_running(struct MarioState *m) {
    return (s16)(-(find_floor_slope(m, 0x0) * m->forwardVel / 40.0f));
}

void play_step_sound(struct MarioState *m, s16 frame1, s16 frame2) {
    if (is_anim_past_frame(m, frame1)
     || is_anim_past_frame(m, frame2)) {
...
void update_sliding_angle(struct MarioState *m, f32 accel, f32 lossFactor) {
    ...
    f32 steepness  = (accel * sqrtf(sqr(floor->normal.x) + sqr(floor->normal.z)));

    m->slideVelX += (steepness * sins(slopeAngle));
    m->slideVelZ += (steepness * coss(slopeAngle));
    ...
    if ((newFacingDYaw > 0x0000) && (newFacingDYaw <= 0x4000)) { // (0x0000..0x4000]
        newFacingDYaw -= 0x200;
        if (newFacingDYaw < 0x0) {
            newFacingDYaw = 0x0;
        }
    } else if ((newFacingDYaw >= -0x4000) && (newFacingDYaw <  0x0000)) { // [-0x4000..0x0000)
        newFacingDYaw += 0x200;
        if (newFacingDYaw > 0x0) {
            newFacingDYaw = 0x0;
        }
    ...
    if (m->forwardVel > 100.0f) {
        f32 divSpeed = (100.0f / m->forwardVel);
        m->slideVelX *= divSpeed;
        m->slideVelZ *= divSpeed;
    }
...
s32 update_sliding(struct MarioState *m, f32 stopSpeed) {
    ...
    if ((oldSpeed > 0.0f) && (newSpeed > 0.0f)) {
        f32 divSpeed = (oldSpeed / newSpeed);
        m->slideVelX *= divSpeed;
        m->slideVelZ *= divSpeed;
    }
...
void tilt_body_walking(struct MarioState *m, s16 startYaw) {
    ...
    if ((animID == MARIO_ANIM_WALKING)
     || (animID == MARIO_ANIM_RUNNING)) {
        s16 dYaw = (m->faceAngle[1] - startYaw);
        //! (Speed Crash) These casts can cause a crash if (dYaw * forwardVel / 12) or (forwardVel * 170) exceed or equal 2^31.
...
void tilt_body_ground_shell(struct MarioState *m, s16 startYaw) {
    ...
    s16 dYaw = m->faceAngle[1] - startYaw;
    //! (Speed Crash) These casts can cause a crash if (dYaw * forwardVel / 12) or (forwardVel * 170) exceed or equal 2^31.
    //! Harder (if not impossible to do) while on a Koopa Shell making this less of an issue.
    s16 nextBodyRoll = -(s16)(dYaw * m->forwardVel / 12.0f);
    s16 nextBodyPitch = (s16)(m->forwardVel * 170.0f);
...
s32 act_butt_slide(struct MarioState *m) {
    s32 cancel = common_slide_action_with_jump(m, ACT_BUTT_SLIDE_STOP,
                                                  ACT_JUMP,
                                                  ACT_BUTT_SLIDE_AIR,
                                                  MARIO_ANIM_SLIDE);
...
s32 act_hold_butt_slide(struct MarioState *m) {
    ...
    s32 cancel = common_slide_action_with_jump(m, ACT_HOLD_BUTT_SLIDE_STOP,
                                                  ACT_HOLD_JUMP,
...
s32 act_crouch_slide(struct MarioState *m) {
    ...
    if (m->input & INPUT_B_PRESSED) {
        if (m->forwardVel >= 10.0f) {
            return set_mario_action(m, ACT_SLIDE_KICK,    0x0);
        } else {
    ...
    return common_slide_action_with_jump(m, ACT_CROUCHING,
                                            ACT_JUMP,
                                            ACT_FREEFALL,
...
s32 common_ground_knockback_action(struct MarioState *m, s32 animation, s32 checkFrame, s32 playLandingSound, s32 actionArg) {
    ...
    m->forwardVel = CLAMP(m->forwardVel, -32.0f, 32.0f);
...
u32 common_landing_action(struct MarioState *m, s16 animation, u32 airAction) {
    ...
    if ((m->floor->type >= SURFACE_SHALLOW_QUICKSAND)
...
s32 act_long_jump_land(struct MarioState *m) {
    ...
    common_landing_action(m, (!m->marioObj->oMarioLongJumpIsSlow ? MARIO_ANIM_CROUCH_FROM_FAST_LONGJUMP
