...
s32 check_kick_or_dive_in_air(struct MarioState *m) {
    if (m->input & INPUT_B_PRESSED) {
        ...
        return set_mario_action(m, m->forwardVel >= 0.0f ? ACT_DIVE : ACT_JUMP_KICK, 0);
    }
    return FALSE;
}
...
s32 check_horizontal_wind(struct MarioState *m) {
    ...
    if (floor->type == SURFACE_HORIZONTAL_WIND) {
        ...
        if (speed > sqr(48.0f)) {
            speed = (48.0f / sqrtf(speed));
            m->slideVelX = (m->slideVelX * speed);
            m->slideVelZ = (m->slideVelZ * speed);
            speed = 48.0f;
        } else if (speed > 32.0f) {
            speed = 32.0f;
        }
...
s32 act_freefall(struct MarioState *m) {
    s32 animation = MARIO_ANIM_SLOW_LEDGE_GRAB; // I guess?
...
s32 act_long_jump(struct MarioState *m) {
    ...
    if ((m->floor->type == SURFACE_VERTICAL_WIND)
     && (m->actionState == ACT_STATE_LONG_JUMP_PLAY_SOUND_HERE_WE_GO)) {
        play_sound(SOUND_MARIO_HERE_WE_GO, m->marioObj->header.gfx.cameraToObject);
        m->actionState = ACT_STATE_LONG_JUMP_PLAYED_SOUND_HERE_WE_GO;
    }
...
s32 act_ground_pound(struct MarioState *m) {
    ...
    if (m->actionState == ACT_STATE_GROUND_POUND_SPIN) {
        if (m->actionTimer < 10) {
        ...
        u32 stepResult = perform_air_step(m, AIR_STEP_CHECK_NONE);
...
s32 act_crazy_box_bounce(struct MarioState *m) {
    ...
    if (m->actionTimer == 0) {
        switch (m->actionArg) {
            case 0: m->vel[1] =  45.0f; minSpeed = 32.0f; break;
            case 1: m->vel[1] =  60.0f; minSpeed = 36.0f; break;
            case 2: m->vel[1] = 100.0f; minSpeed = 48.0f; break;
        }
    ...
    switch (stepResult) {
        ...
        case AIR_STEP_LANDED:
            ...
            if (!check_fall_damage_or_get_stuck(m, hardFallAction)) {
                if ((m->action == ACT_THROWN_FORWARD)
        ...
        case AIR_STEP_LANDED:
            if ((m->actionState == ACT_STATE_BUTT_SLIDE_AIR_SMALL_BOUNCE)
             ...
             && (m->floor->normal.y >= COS10)) {
                ...
                m->actionState = ACT_STATE_BUTT_SLIDE_AIR_START_SLIDING;
            } else {
...
s32 act_riding_hoot(struct MarioState *m) {
    if (!(m->input & INPUT_A_DOWN)
     || (m->marioObj->oInteractStatus & INT_STATUS_MARIO_DROP_FROM_HOOT)) {
    ...
    vec3f_copy_y_off(m->pos, &m->usedObj->oPosVec, -92.5f);
...
s32 act_flying_triple_jump(struct MarioState *m) {
    ...
    if (m->actionState == ACT_STATE_FLYING_TRIPLE_JUMP_SPIN
