...
ALIGNED16 static s16 hvqm_pcmbuf[NUM_PCMBUFs][PCMBUF_SIZE];
...
static void timekeeperProc(void UNUSED *argument) {
    ...
    for (;;) {
        ...
        /* hijack vi/ai event only when we start playing! */
        ...
        while (!video_done || videoRingCount > 0 || pushed_cfb != NULL || !audio_done
            ...
            if (!audio_done && audioRingCount < AUDIO_RING_BUFFER_SIZE
                ...
                samples = audioproc(&hvqm_pcmbuf[next_pcmbufno][pcm_mod_samples << PCM_CHANNELS_SHIFT]);

                if (samples > 0) {
                    ++pcmBufferCount;

                    sp = pcmModBuf;
                    dp = (s16 *) ((u8 *) hvqm_pcmbuf[next_pcmbufno]);
                    ...
                    buffer = hvqm_pcmbuf[next_pcmbufno];
                    ...
                    sp = (s16 *) ((u8 *) (hvqm_pcmbuf[next_pcmbufno]) + length);
                    dp = pcmModBuf;
                    i = pcm_mod_samples << PCM_CHANNELS_SHIFT;
                    while (i--)
                        *dp++ = *sp++;
        ...
        /* restore vi vblank event back to the game! */
        osViSetEvent(&gIntrMesgQueue, (OSMesg) 102, 1);

        /* notify tkResMesgQ that we are stopped! */
        osSendMesg(&tkResMesgQ, NULL, OS_MESG_BLOCK);
    }
}
