...
#define AUDIO_SAMPLES_MAX                                                                              \
    (((AUDIO_RECORD_SIZE_MAX - sizeof(HVQM2Audio)) * 8 / AUDIO_SAMPLE_BITS)                            \
     + 1) /* Maximum number of records per sample */

/*
 * PCM buffer specifications
 */
#define NUM_PCMBUFs 3 /* Number of PCM buffers (2 or more, at least 3 recommended) */
#define PCMBUF_SPREAD                                                                                  \
    ((PCM_ALIGN - 1)                                                                                   \
     + AUDIO_SAMPLES_MAX) /* Minimum required capacity for PCM buffer = Number of samples carried      \
                             forward from last time + number of samples newly decoded */
#define PCMBUF_ALIGNED                                                                                 \
    ((PCMBUF_SPREAD + (PCM_ALIGN - 1)) & (~(PCM_ALIGN - 1))) /* pcmbuf[i] address is aligned */
