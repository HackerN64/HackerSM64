 ...
 *
 * NOTE: Original macros had conflicts, so directly dereference properly
 */
#undef load32
#undef load16
...
static tkAudioProc rewind(void) {
    video_streamP = audio_streamP = gHvqmRomStart + sizeof(HVQM2Header);
...
OSMesgQueue hvqmMesgQ;
OSMesg hvqmMesgBuf;

OSThread hvqmThread;
static u64 hvqmStack[STACKSIZE / sizeof(u64)];

OSMesgQueue gHvqmDoneQueue;
static OSMesg gHvqmDoneMesgBuf;
volatile s32 gHvqmPlayState = 0;
...
void hvqm_main_proc() {
    ...
    for (;;) {
        OSMesg start_msg;
        osRecvMesg(&hvqmMesgQ, &start_msg, OS_MESG_BLOCK);

        gHvqmRomStart = (u8 *) start_msg;
        ...
        romcpy(hvqm_header, (void *) gHvqmRomStart, sizeof(HVQM2Header), OS_MESG_PRI_NORMAL,
        ...
        h_offset = (SCREEN_WD - load16(hvqm_header->width)) / 2;
        v_offset = (SCREEN_HT - load16(hvqm_header->height)) / 2;
        screen_offset = v_offset * SCREEN_WD + h_offset;
        ...
        tkStop();
        osSetEventMesg(OS_EVENT_SP, &gIntrMesgQueue, (OSMesg) 100);
        osAiSetFrequency(gAiFrequency);
        osSendMesg(&gHvqmDoneQueue, (OSMesg) 1, OS_MESG_NOBLOCK);
    }
}

void createHvqmThread(void) {
    osCreateMesgQueue(&hvqmMesgQ, &hvqmMesgBuf, 1);
    osCreateMesgQueue(&gHvqmDoneQueue, &gHvqmDoneMesgBuf, 1);
...
void play_hvqm(u8 *romStart) {
    if (gHvqmPlayState == 0) {
        ...
        turn_off_audio();
        gHvqmPlayState = 2;
        osSendMesg(&hvqmMesgQ, (OSMesg) romStart, OS_MESG_BLOCK);
        osRecvMesg(&gHvqmDoneQueue, NULL, OS_MESG_BLOCK);
        gHvqmPlayState = 0;
        turn_on_audio();
        osSendMesg(&gGfxVblankQueue, gMainReceivedMesg, OS_MESG_NOBLOCK);
    }
}
